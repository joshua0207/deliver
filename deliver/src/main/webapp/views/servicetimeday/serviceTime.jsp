...
<html>
	...
	<script type="text/javascript">
		...
		function check() {
			if (!chkChkBoxed()) {
				alert('<spring:message code="LAB1023" />');
				//} else if (!checkTime()) {
				...
				//alert(getCheckedVal());
				$('#updateServiceTimeForm').append(hdnObj);
				$("#updateServiceTimeForm").submit();
			}
		}
		...
		//新增營業時間
		function addDate() {
			var serviceTimeWeekTr;
			var serviceTimeListTr;
			var serviceTimeChkBox;
			var temp;
	
			//先分別烤貝"營業時間_周"的上一層Tr及"營業時間"的上一層Tr
			serviceTimeWeekTr = $("#serviceTimeTabTemp").children().children("tr:first-child").clone();
			serviceTimeListTr = $("#serviceTimeTabTemp").children().children("tr:first-child").next("tr").clone();
			serviceTimeListTr2 = $("#serviceTimeTabTemp").children().children("tr:first-child").next("tr").next("tr").clone();
	
			//取得該Tr中的checkbox
			serviceTimeChkBox = serviceTimeWeekTr.children("td:last-child");
			
			//更改checkbox的id及name，名字只要別和其它的checkbox重覆即可
			serviceTimeChkBox.children().each(function(index) {
				if ($(this).is(':checkbox')) {
					temp = "listWeek_" + $("#serviceTimeTab").children().children("tr").length;
					//alert(temp + "-" + index);
					//alert(temp);
			...
			//新增至列表中
			serviceTimeWeekTr.appendTo($("#serviceTimeTab"));
			serviceTimeListTr.appendTo($("#serviceTimeTab"));
			serviceTimeListTr2.appendTo($("#serviceTimeTab"));
		}
                 ...
                 <div id="cont_right">  
                   ...
                   <div class="serviceTermForm">
						...
						<div class="shopDataForm">
							<form name="updateServiceTimeForm" id="updateServiceTimeForm"
								action="servicetime/shop/addservicetimeday" method="post">
								
								<table id="serviceTimeTab" border="0" cellspacing="0"
									cellpadding="0" width="100%">
									
									
									<c:forEach items="${serviceTimeList}" var="time" varStatus="s">
									<script type="text/javascript">
										timeObject = '${time.serviceDay}';
										timeList[timeList.length] = timeObject;
									</script>
									
									<tr>
										<td class="embg3"><spring:message code="LAB1026" /></td>
									...
									<tr>
										<td class="embg3"><font size="2"><spring:message code="LAB1028" /></font></td>
										<td class="fillembg">
										<select name="listTimeTwentyFourStart" value="07"
											id="listTimeTwentyFourStart" >
												<c:forEach var="i" begin="0" end="23">
													<c:choose>
														<c:when test="${i < 10}">
															<c:choose>
																<c:when test="${time.serviceTimeBegin == i}">
											...
											id="listTimeMinuteStart">
												<c:choose>
													<c:when test="${time.serviceMinuteBegin == 30}">
											...
											id="listTimeTwentyFourEnd">
												<c:forEach var="i" begin="0" end="23">
													<c:choose>
														<c:when test="${i < 10}">
															<c:choose>
																<c:when test="${time.serviceTimeEnd == i}">
																	<option value="0${i}" selected="selected">0${i}</option>
																</c:when>
																<c:otherwise>
																	<option value="0${i}">0${i}</option>
											...
											<select name="listTimeMinuteEnd" value="00" id="listTimeMinuteEnd">
												<c:choose>
													<c:when test="${time.serviceMinuteEnd == 30}">
														<option value="00">00</option>
														<option value="30" selected="selected">30</option>
													</c:when>
													<c:otherwise>
						...
						<table id="serviceTimeTabTemp" border="0" cellspacing="0"
